...
std::string TICKER = "GOOGL"; // actual definition
    ...
    if (users.find(userId1) != users.end() && users.find(userId2) != users.end())
    {
        if (users[userId1].user_balance.balance["USD"] >= price * quantity)
        {
            if (users[userId2].user_balance.balance[TICKER] >= quantity)
            {
                ...
                std::cout << "Funds and stocks transfered!" << endl;
            }
            else
            {
                std::cout << "User does not have enough balance to sell the stock" << endl;
            }
        }
        else
        {
            std::cout << "User does not have enough balance to buy the stock" << endl;
        }
    }
    else
    {
        std::cout << "One or both users not found" << endl;
    }
}
...
std::string OrderBook::add_bid(std::string Username, int Price, int Quantity)
{
    ...
    if (remQty > 0)
    {
        ...
        bids.push_back(bid);
        std::cout << "Remaining quantity of bids added to Orderbook" << endl;
    }

    if (remQty == 0)
    {
        std::cout << "Complete Bid Satisfied Successfully" << endl;
    }
...
std::string OrderBook::add_ask(std::string Username, int Price, int Quantity)
{
    ...
    if (remQty > 0)
    {
        ...
        asks.push_back(ask);
        std::cout << "Remaining quantity of asks added to Orderbook" << endl;
    }

    if (remQty == 0)
    {
        std::cout << "Complete Ask Satisfied Successfully" << endl;
    }
...
std::string OrderBook::getBalance(std::string username)
{
    ...
    if (users.find(username) != users.end())
    {
        ...
        std::cout << "User balance is as follows: " << endl;
...
std::string OrderBook::getQuote(int qty)
{
    ...
    std::cout << "Quote retrieved successfully." << endl;
    return "Quote retrieved successfully.";
}
...
std::string OrderBook::addBalanace(std::string Username, std::string market, int value)
{
    if (users.find(Username) != users.end())
    {
        ...
        std::cout << "Balance added successfully" << endl;
        return "Balance added successfully";
    }
    std::cout << "User not found!! Please enter the right Username to add balance!" << endl;
    return "User not found";
}
...
void OrderBook::cancelAsk(std::string Username, int Price, int Quantity) {
    ...
    for (auto it = asks.begin(); it != asks.end(); ++it) {
        if (it->user_name == Username && it->price == Price && it->quantity == Quantity) {
            asks.erase(it);
            std::cout << "Ask cancelled successfully" << endl;
            return;
        }
            ...
            std::cout << "Ask quantity is less than the quantity you want to cancel\n\n";
            std::cout << "Please enter the right quantity to cancel and retry!\n\n";
            return;
...
void OrderBook::cancelBid(std::string Username, int Price, int Quantity) {
    ...
    for (auto it = bids.begin(); it != bids.end(); ++it) {
        if (it->user_name == Username && it->price == Price && it->quantity == Quantity) {
            bids.erase(it);
            std::cout << "Bid cancelled successfully" << endl;
            return;
        }
        else if (it->user_name == Username && it->price == Price && it->quantity > Quantity) {
            it->quantity -= Quantity;
            ...
            std::cout << "Bid quantity is less than the quantity you want to cancel\n\n";
            std::cout << "Please enter the right quantity to cancel and retry!\n\n";
            return;
            // RETHINK MAYBE - I will not return I will continue search as maybe some other order was placed by the same user with the same price and quantity
    ...
    std::cout << "Bid not found!! Please enter the right Username, Price and Quantity to cancel the bid!" << endl;
    return;
}
...
int test()
{
    ...
    while (true)
    {
        ...
        std::cout << "4. Add Bid to " << TICKER << " v USD market\n";
        std::cout << "5. Sell your stocks in " << TICKER << " v USD Market\n";
        std::cout << "6. Get Current Quote to buy " << TICKER << " stocks\n";
